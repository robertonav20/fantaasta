          ...
          <TableBody>
            {ROLE_ORDER.flatMap((role) => [
              <TableRow key={`${role}-header`} sx={{ bgcolor: 'rgba(88,166,255,.09)' }}>
                <TableCell colSpan={6} sx={{ fontWeight: 700 }}><Stack direction="row" style={{alignItems: 'center'}} spacing={.75}><RoleBadge role={role} /><span>{ROLE_NAMES[role]} · {metrics.role[role].filled}/{LIMITS[role]}</span></Stack></TableCell>
              </TableRow>,
              ...state.slots[role].map((slot, index) => {
                ...
                return (
                  <TableRow key={`${role}-${index}`} hover sx={{ '&:nth-of-type(even)': { bgcolor: 'rgba(88,166,255,.025)' } }}>
                    ...
                    <TableCell align="right" sx={{ fontWeight: 700 }}>{player ? money(player.fvm) : '—'}</TableCell>
                    <TableCell align="center"><TextField size="small" type="number" value={slot.actual} onChange={(e) => onPatchSlot(role, index, { actual: Number(e.target.value || 0) })} sx={{ width: 100, '& input': { textAlign: 'center', fontWeight: 700, color: actualTone(Number(slot.actual || 0), expected) }, '& .MuiOutlinedInput-notchedOutline': { borderColor: actualTone(Number(slot.actual || 0), expected) } }} /></TableCell>
